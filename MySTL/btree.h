...
//    linear_search_type, binary_search_type>::type search_type;
   ...
   typedef binary_search_type search_type;
  struct base_fields {
 ...
 public:
  ...
  static btree_node* init_leaf(
    ...
    f->count = 0;
    f->parent = parent;
    printf("%s: btree_node=%x, parent=%x \n", __func__, n, parent);
    if (!NDEBUG) {
...
public:
  ...
  node_type* new_leaf_root_node(int max_count) {
    ...
    printf("%s: %d \n", __func__, sizeof(base_fields) + max_count * sizeof(value_type));
...
void btree_node<P>::rebalance_right_to_left(btree_node *src, int to_move) {
  ...
  // 现在左侧节点 count() 之后初始化 to_move 个元素
...
btree<P>::insert_unique(const key_type &key, ValuePointer value) {
  ...
  std::pair<iterator, int> res = internal_locate(key, iterator(root(), 0));
    printf("%s: iterator position = %d, res.second=%d \n", __func__, res.first.position, res.second);
  ...
  if (res.second == kExactMatch) {
    ...
    if(last.node == NULL) printf("%s: last.node is NULL \n", __func__);
...
bool btree<P>::try_merge_or_rebalance(iterator *iter) {
  ...
  if (iter->node->position() < parent->count()) {
    ...
    // 在erase 函数中，iter 执行remove_value 后，iter.position = iter.node->count
...
inline IterType btree<P>::internal_last(IterType iter) {
  while (iter.node && iter.position == iter.node->count()) {
    iter.position = iter.node->position();
      printf("%s: position=%d \n", __func__, iter.position);
    iter.node = iter.node->parent();
    if (iter.node->leaf()) {
        printf("%s: set node is NULL. \n", __func__);
      iter.node = NULL;
    }
  }
  return iter;
...
btree<P>::internal_insert(iterator iter, const value_type &v) {
    printf("%s: leaf = %d \n", __func__, iter.node->leaf());
    if (!iter.node->leaf()) {
      printf("%s: node is not leaf. \n", __func__);
